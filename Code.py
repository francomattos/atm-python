...
#Creates initial frame and its parameters
mainFrame = Tk()
...
accountTypes = ("Checking", "Savings", "General Loan", "Mortgage", "Car Loan", "Boat Loan", "Credit Card")
...
if not os.path.exists(dataFileName):
    with open(dataFileName, 'w'): pass

#Function validating only float input
...
def new_User(event):
    def create_User(accountType, usrName, usrPin, initBalance):
        if len(accountType) == 0 or len(usrName) == 0 or len(usrPin) == 0 or len(initBalance) == 0:
            tkinter.messagebox.showwarning(title="Form Imcomplete", message="Please complete form prior to creating an account.", parent=newUserFrame)
        else:
            endOfDay = datetime.combine(date.today() + timedelta(days=1), datetime.min.time())
            try:
            ...
            if usrName in jsonData:
                tkinter.messagebox.showwarning(title="User not unique", message="There is an user already under your name.", parent=newUserFrame)
            else:
                ...
                tkinter.messagebox.showinfo(title="User Created", message="Your user has been created, you may now login.", parent=mainFrame)
    ...
    newBalance = Entry(newUserFrame, width=15, validate="key", validatecommand=(newUserFrame.register(validate_Float), '%P'))
    ...
    closeBtn = Button(newUserFrame, text="Cancel", padx=10, command=newUserFrame.destroy)
...
def open_Account(_userName):
    ...
    def make_Deposit():
        ...
        if jsonData[_userName]["Accounts"][accNum]["TransactionCount"] >= 10:
            tkinter.messagebox.showwarning(title="warning", message="Maximum of 10 transactions per account per day.", parent=userAccountFrame)
            return
        if len(transactionAmount.get()) == 0:
            tkinter.messagebox.showwarning(title="warning", message="Please enter a transaction amount.", parent=userAccountFrame)
            return
        jsonData[_userName]["Accounts"][accNum]["AccountBalance"] += float(transactionAmount.get())
        jsonData[_userName]["Accounts"][accNum]["TransactionCount"] += 1
        accountBalance.set("$" + str(jsonData[_userName]["Accounts"][accNum]["AccountBalance"]))
        with open(dataFileName,"w") as jsonFile:
            json.dump(jsonData, jsonFile, indent=2)
        tkinter.messagebox.showinfo(title="Complete", message="Your transaction is complete!", parent=userAccountFrame)
    ...
    def make_Withdraw():
        ...
        if len(transactionAmount.get()) == 0:
            ...
            tkinter.messagebox.showwarning(title="warning", message="You may only withdraw up to $500.", parent=userAccountFrame)
            return
        if jsonData[_userName]["Accounts"][accNum]["AccountBalance"] < float(transactionAmount.get()):
            tkinter.messagebox.showwarning(title="warning", message="Not enough funds for your transaction.", parent=userAccountFrame)
            return
        jsonData[_userName]["Accounts"][accNum]["AccountBalance"] -= float(transactionAmount.get())
        jsonData[_userName]["Accounts"][accNum]["TransactionCount"] += 1
        accountBalance.set("$" + str(jsonData[_userName]["Accounts"][accNum]["AccountBalance"]))
    ...
    def transfer_Funds():
        def make_Transfer():
            ...
            if jsonData[_userName]["Accounts"][fromAccNum]["AccountBalance"] < float(transferAmount.get()):
                tkinter.messagebox.showwarning(title="Not Enough Funds", message="You do not have enough funds to make this transfer.", parent=transferFundFrame)
                return
            toAccIndex = usrAccountTypes.index(toAccount.get())
            toAccNum = accountNumbers[toAccIndex]
            jsonData[_userName]["Accounts"][fromAccNum]["AccountBalance"] -= float(transferAmount.get())
            jsonData[_userName]["Accounts"][toAccNum]["AccountBalance"] += float(transferAmount.get())
            accIndex = usrAccountTypes.index(accountType.get())
            accNum = accountNumbers[accIndex]
            accountBalance.set("$" + str(jsonData[_userName]["Accounts"][accNum]["AccountBalance"]))
            with open(dataFileName,"w") as jsonFile:
                json.dump(jsonData, jsonFile, indent=2)
            tkinter.messagebox.showinfo(title="Complete", message="Your transfer is completed!", parent=transferFundFrame)
            transferFundFrame.destroy()

        if len(accountNumbers) == 1:
            tkinter.messagebox.showwarning(title="Not Enough Accounts", message="You can only make internal transfers, please create another account.", parent=transferFundFrame)
            return
        ...
        fromAccountType = OptionMenu(transferFundFrame, fromAccount, *usrAccountTypes)
        fromAccountType.place(x=95, y=6)
        accLabel = Label(transferFundFrame, text="Amount:")
        accLabel.place(x=260, y=10)
        transferAmount = Entry(transferFundFrame, width=12, validate="key", validatecommand=(transferFundFrame.register(validate_Float), '%P'))
        ...
        toAccountType = OptionMenu(transferFundFrame, toAccount, *usrAccountTypes)
        toAccountType.place(x=95, y=46)

        transferBtn = Button(transferFundFrame, text="Make Transfer", padx=5, command=lambda : make_Transfer())
        transferBtn.place(x=40, y=95)
        closeBtn = Button(transferFundFrame, text="Cancel", padx=5, command=transferFundFrame.destroy)
    ...
    #Function for deleting account or user
    def delete_Account(_accountType):
        ...
        deleteBol = tkinter.messagebox.askquestion(title="Close account", message="Would you like to delete this account? \nYour current balance is: ${}".format(closeBal), parent=userAccountFrame)
        if deleteBol:
            if len(accountNumbers) == 1:
                deleteusr = tkinter.messagebox.askquestion(title="Delete User", message="This is your last account! \nIf remove this account, your user will be delete.\nShould we proceed?", parent=userAccountFrame)
                if deleteusr:
                    del jsonData[_userName]
                    with open(dataFileName,"w") as jsonFile:
                        json.dump(jsonData, jsonFile, indent=2)
                    tkinter.messagebox.showinfo(title="User Deleted", message="Your user has been deleted.", parent=mainFrame)
                    close_Window()
            else:
                del jsonData[_userName]["Accounts"][accNum]
                with open(dataFileName,"w") as jsonFile:
                    json.dump(jsonData, jsonFile, indent=2)
                tkinter.messagebox.showinfo(title="Account Deleted", message="Your account has been deleted.", parent=mainFrame)
    ...
    def new_Account(_userName):
        def create_Account():
            if len(transactionAmount.get()) == 0:
                tkinter.messagebox.showwarning(title="Form Imcomplete", message="Please enter an initial balance.", parent=newAccFrame)
            else:
                with open(dataFileName, 'r') as jsonFile:
                    jsonData = json.load(jsonFile)
                ...
                with open(dataFileName,"w") as jsonFile:
                    json.dump(jsonData, jsonFile, indent=2)
                tkinter.messagebox.showwarning(title="Account Created", message="The new account has been assigned to you.", parent=newAccFrame)
                newAccFrame.destroy()
                userAccountFrame.destroy()
                open_Account(_userName)
        ...
        selectaccountType = OptionMenu(newAccFrame, accountType, *accountTypes, command=lambda account: init_newAccount(account))
        ...
        transactionAmount = Entry(newAccFrame, width=12, validate="key", validatecommand=(newAccFrame.register(validate_Float), '%P'))
        transactionAmount.place(x=10, y=73)
        transactionAmount.focus()

        createBtn = Button(newAccFrame, text="Create Account", padx=10, command=lambda : create_Account())
        createBtn.place(x=100, y=70)
        closeBtn = Button(newAccFrame, text="Cancel", padx=10, command=newAccFrame.destroy)
    ...
    selectaccountType = OptionMenu(userAccountFrame, accountType, *usrAccountTypes, command=lambda account: change_Account(account))
    selectaccountType.place(x=90, y=6)
    ...
    accountBalance.set("$" + str(jsonData[_userName]["Accounts"][accountNumbers[0]]["AccountBalance"]))
    ...
    transactionAmount = Entry(userAccountFrame, width=12, validate="key", validatecommand=(userAccountFrame.register(validate_Float), '%P'))
    transactionAmount.place(x=67, y=53)
    transactionAmount.focus()

    depositBtnTxt = StringVar(userAccountFrame)
    depositBtn = Button(userAccountFrame, textvariable=depositBtnTxt, padx=10, command=lambda : make_Deposit())
    depositBtn.place(x=150, y=50)
    withdrawBtnTxt = StringVar(userAccountFrame)
    withdrawBtn = Button(userAccountFrame, textvariable=withdrawBtnTxt, padx=5, command=lambda : make_Withdraw())
    withdrawBtn.place(x=240, y=50)
    transferFundBtn = Button(userAccountFrame, text="Transfer Funds", padx=15, command=lambda : transfer_Funds())
    transferFundBtn.place(x=10, y=90)

    accLabel = Label(userAccountFrame, text="Create New Account.", fg="blue", cursor="hand2")
    accLabel.place(x=190, y=90)
    accLabel.bind("<Button-1>", lambda b: new_Account(_userName))

    deleteAcctBtn = Button(userAccountFrame, text="Delete Account", padx=15, command=lambda : delete_Account(accountType.get()))
    deleteAcctBtn.place(x=190, y=120)

    #Reset end of day on account
    endOfDay = datetime.strptime(jsonData[_userName]["Accounts"][accountNumbers[0]]["EndOfBusinessDay"], "%Y-%m-%d %H:%M:%S.%f") 
    if datetime.now() > endOfDay:
        endOfDay = datetime.combine(date.today() + timedelta(days=1), datetime.min.time())
        for x in jsonData[_userName]["Accounts"]:
            jsonData[_userName]["Accounts"][x]["TransactionCount"] = 0
            jsonData[_userName]["Accounts"][x]["EndOfBusinessDay"] = datetime.strftime(endOfDay, "%Y-%m-%d %H:%M:%S.%f")
        with open(dataFileName,"w") as jsonFile:
                    json.dump(jsonData, jsonFile, indent=2)

    init_Layout(jsonData[_userName]["Accounts"][accountNumbers[0]]["AccountType"])
...
def new_Login(_userName, _usrPin):
    if _userName and _usrPin:
        try:
            ...
            tkinter.messagebox.showwarning("Login Failed","Please create an account.")
            return
        if _userName in jsonData and jsonData[_userName]["Pin"] == _usrPin:
            mainFrame.withdraw()
            open_Account(_userName)
        else:
            tkinter.messagebox.showwarning("Login Failed","Please enter correct username and pin.")
    else:
        tkinter.messagebox.showwarning("Login Failed","Please enter correct username and pin.")

mainLabels = Label(mainFrame, text="Please enter your full name and pin to sign into your account.")
...
loginBtn = Button(mainFrame, text="Login", padx=10, command=lambda : new_Login(usrName.get(), usrPin.get()))
loginBtn.place(x=60, y=85)
closeBtn = Button(mainFrame, text="Close", padx=10, command=mainFrame.destroy)
closeBtn.place(x=140, y=85)

mainLabels = Label(mainFrame, text="Create New User.", fg="blue", cursor="hand2")
